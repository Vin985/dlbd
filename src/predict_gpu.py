...
src_root = pathlib.Path("/home/sylvain/data/datasets/tommy")
dest_root = pathlib.Path("/home/sylvain/data/results/tommy")


file_list_path = src_root / "recording_list_full.csv"
...
for plot_id in file_list.plot_id.unique():
    ...
    res.append(tmp_df)
    tmp_df.to_feather(
        str(ensure_path_exists(dest_root / "intermediate" / (plot_id + ".feather")))
    )
    print("Classified {0} in {1}".format(plot_id, time() - tic))


res_df = pd.concat(res).reset_index()
res_df.to_feather(str(ensure_path_exists(dest_root / "predictions_all.feather")))
