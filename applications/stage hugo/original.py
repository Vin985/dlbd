...
dir_root =  pathlib.Path(r"C:\Users\zepht\Desktop\Test_dossier") #askdirectory()      #Selection du dossier dans lequel appliquer le traitement                   
...
def Assemblage():
    res = []
    #path_windows1 = pathlib.Path(name1)                                     #Recuperation du chemin d'accès de la sauvegarde
    for root,dirs,files in os.walk(dir_root) :                               #Automatisation par loop. Il faut trouver un moyen de pouvoir selection le dossier sur lequel on va appliquer le script
        for file in files :                                                  #Pour chaque fichier dans le dossier:
            if file.endswith(".csv") :                                       #Si le fichier est une extension.csv alors:     
                try:                      
                    ...
                    PathPlot = path_windows.parts[-2]                       #On  and convertit le plot et l'année du dossier contenant le fichier en variable
    ...
    df_final = pd.concat(res).drop_duplicates().reset_index(drop=True)    #Concaténation sans doublon
    df_gb = df_final.groupby(["Label","Related"]).sum()                     #Enregistrement d'un csv brute de Label et de Related
...
def Sort () :   
    ...
    for (Label,Related) in df_sort.itertuples(index=False):
        df.loc[df.Label == Label,"Related"] = Related                   #Modification du Related en fonction du Label de reference
    df_final_sort=df
...
def Stat_Mean () :
    ...
    try :
        ...
        pass     
    df_Stat = df_Stat_Mean.loc[:,["MaxAmp","MinAmp","MeanAmp","MinimumFreq_Hz","MaximumFreq_Hz"]].groupby(df_Stat_Mean["Label"]).mean() 
    df_Stat.columns = ['MaxAmp_Mean', 'MinAmp_Mean', 'MeanAmp_Mean','MinimumFreq_Hz_Mean','MaximumFreq_Hz_Mean']
...
def Stat_Max () :
    ...
    try :
        ...
        pass     
    df_Stat = df_Stat_Max.loc[:,["MaxAmp","MinAmp","MeanAmp","MinimumFreq_Hz","MaximumFreq_Hz"]].groupby(df_Stat_Max["Label"]).max() 
    df_Stat.columns = ['MaxAmp_Max', 'MinAmp_Max', 'MeanAmp_Max','MinimumFreq_Hz_Max','MaximumFreq_Hz_Max']
...
def Stat_Min():
    ...
    try :
        ...
        pass     
    df_Stat = df_Stat_Min.loc[:,["MaxAmp","MinAmp","MeanAmp","MinimumFreq_Hz","MaximumFreq_Hz"]].groupby(df_Stat_Min["Label"]).min()
    df_Stat.columns = ['MaxAmp_Min', 'MinAmp_Min', 'MeanAmp_Min','MinimumFreq_Hz_Min','MaximumFreq_Hz_Min']
...
def Stat_Std():
    ...
    try :
        ...
        pass     
    df_Stat_Std["Sing_Time_Seconds"]=df_Stat_Std["LabelEndTime_Seconds"]-df_Stat_Std["LabelStartTime_Seconds"] #Creation d'une colonne qui recup le temps de chaque chant
    df_Stat = df_Stat_Std.loc[:,["Sing_Time_Seconds"]].groupby(df_Stat_Std["Label"]).std()
    df_Stat.columns = ['Std']
...
def Stat_Global():
    ...
    try :
        ...
        pass     
    df_Stat = df_Stat_Global.loc[:,["LabelStartTime_Seconds","LabelEndTime_Seconds"]].groupby(df_Stat_Global["Label"]).sum()
    df2=df_Stat_Global["Label"].value_counts()  
    frames=[df_Stat,df2] #Assemblage
    A,B,C,D = Stat_Min(),Stat_Max(),Stat_Mean(),Stat_Std()
    result=pd.concat(frames,axis=1).drop_duplicates().reset_index(drop=True)    #Assemblage concaténation
    result["Total_Sing"]=result["LabelEndTime_Seconds"]-result["LabelStartTime_Seconds"] #Durée total de chant
    ...
    result3.to_csv(f'{name5/Essai4}.csv', index=True)    #Enregistrement du fichier    #Enregistrement du fichier
...
def Stat_Label():  
    try:
        ...
        df3 = df.loc[:,["LabelStartTime_Seconds","LabelEndTime_Seconds"]].groupby(df["Label"]).sum()
        ...
        (A,B,C) = (round(Noice1["%Noice"],2),round(Noice2["%Noice"],2),round(Noice3["%Noice"],2))
        (df3["Rain"],df3["Rain%"],df3["Wind"],df3["Wind%"],df3["Both"],df3["Both3%"])=(Noice1["noise"],A,Noice2["noise"],B,Noice3["noise"],C)     
        ...
        fenetre = Tk()
        label = Label(fenetre, text="Unknown keyword, check the writing and be careful with the case")
        label.pack()
